/**
 * Server-side object storage helpers for Nuxt / Nitro event handlers.
 *
 * createServerClient() returns a LoomupClient with `client.storage`. These
 * helpers convert multipart FormData and build download Responses.
 */
...
export function fileAndPathFromFormData(
  ...
  if (!isFileLike(raw)) {
    throw new Error(
      `@loomup/nuxt: FormData field "${fileField}" must be a File/Blob`,
    );
  }
  let path =
    options?.path ??
  ...
  if (!path || !path.trim()) {
    throw new Error(
      `@loomup/nuxt: object path required (pass options.path, form field "${pathField}", or a named File)`,
    );
  }
  path = path.replace(/^\/+/, "");
...
/**
 * Upload from multipart FormData (e.g. `readMultipartFormData` → FormData, or
 * `readFormData(event)` on Nitro).
 */
...
/** Stream a stored object as a Web Response for the event handler return value. */
