...
/** Stable serialization for composable dependency keys. */
...
export function applyChangeToRows<T extends Record<string, unknown>>(
  ...
  if (op === "DELETE") {
    ...
    return rows.filter(
      (r) =>
        rowIdFrom(r as Record<string, unknown>, undefined, primaryKey) !== id,
    );
  }
  ...
  if (op === "INSERT" || op === "UPDATE" || op === "RESYNC") {
    ...
    const idx = rows.findIndex(
      (r) =>
        rowIdFrom(r as Record<string, unknown>, undefined, primaryKey) ===
        nextId,
    );
    if (idx === -1) {
      return [...rows, next];
